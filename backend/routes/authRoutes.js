 ...
 *
 * Make sure GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, GOOGLE_REDIRECT_URI are in .env
 */
...
router.get('/google/callback', async (req, res) => {
  try {
    ...
    const frontend = process.env.FRONTEND_URL || 'http://localhost:8501';
    // Provide a simple success page and next steps
    res.send(`<h3>Google Calendar connected ✅</h3><p>Close this window and return to the demo frontend: ${frontend}</p>`);
  } catch (err) {
